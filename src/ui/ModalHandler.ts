...
export class ModalHandler {
  ...
  async showRecruitmentModal(
    ...
    try {
      ...
      await SafeInteraction.safeReply(interaction, 
        SafeInteraction.createErrorResponse('모달 표시', error as Error)
      );
    }
  }
  ...
  async handleModalSubmit(interaction: ModalSubmitInteraction): Promise<ModalHandleResult> {
    ...
    try {
      ...
      if (!validation.valid) {
        ...
        return this.recordSubmissionResult(interaction, 'validation', false, startTime, { errors: validation.errors });
      }
      ...
      if (customId === 'standalone_recruitment_modal') {
        ...
        return this.recordSubmissionResult(interaction, 'standalone', result.success, startTime, result);
        ...
        const result = await this.handleVoiceChannelRecruitment(interaction, recruitmentData, voiceChannelId);
        return this.recordSubmissionResult(interaction, 'voiceChannel', result.success, startTime, result);
      } else {
      ...
      await SafeInteraction.safeReply(interaction, 
        SafeInteraction.createErrorResponse('모달 처리', error as Error)
      );

      return this.recordSubmissionResult(interaction, 'error', false, startTime, { error: errorMsg });
    }
  }
  ...
  private async handleStandaloneRecruitment(
    ...
    try {
      ...
      // 독립 포럼 포스트 생성
      const createResult = await this.forumPostManager.createForumPost(recruitmentData);
      ...
      if (createResult.success && createResult.postId) {
        ...
        console.log(`[ModalHandler] 독립 구인구직 생성 완료: ${recruitmentData.title} (ID: ${createResult.postId})`);
  ...
  validateModalData(recruitmentData: RecruitmentData): ValidationResult {
    ...
    // 태그 검증
    if (recruitmentData.tagsArray && recruitmentData.tagsArray.length > RecruitmentConfig.MAX_SELECTED_TAGS) {
      errors.push(`역할 태그는 최대 ${RecruitmentConfig.MAX_SELECTED_TAGS}개까지 선택할 수 있습니다.`);
    }
  ...
  getUserSubmissionStats(userId: string): {
    ...
    return {
      ...
      successRate: userSubmissions.length > 0 ? (successfulSubmissions.length / userSubmissions.length) * 100 : 0
    };
  }
  ...
  healthCheck(): {
    ...
    const successRate = this.modalStats.totalSubmissions > 0 
      ? (this.modalStats.successfulSubmissions / this.modalStats.totalSubmissions) * 100 
      : 100;

    const validationErrorRate = this.modalStats.totalSubmissions > 0

...
export class GapCalendarCommand extends CommandBase {
  ...
  buildSlashCommand(): SlashCommandBuilder {
    return new SlashCommandBuilder()
      ...
      .addStringOption((option) =>
        option.setName('start_date').setDescription('시작 날짜 (YYYY-MM-DD 형식)').setRequired(true)
      )
      .addStringOption((option) =>
        option.setName('end_date').setDescription('종료 날짜 (YYYY-MM-DD 형식)').setRequired(true)
      )
      .addStringOption((option) =>
        option
          .setName('role')
      ...
      .addBooleanOption((option) =>
        option.setName('include_details').setDescription('상세 정보 포함 여부').setRequired(false)
      )
      .addBooleanOption((option) =>
        option
  ...
  protected async executeCommand(
    ...
    try {
      ...
      // 로그 기록
      if (this.logService) {
        this.logService.logActivity(`달력 보고서 생성`, [interaction.user.id], 'calendar_report', {
      ...
      const errorMessage =
        error instanceof Error ? error.message : '활동 보고서 생성 중 오류가 발생했습니다.';
  ...
  private validateDates(
    ...
    const daysDiff = Math.ceil(
      (parsedEndDate.getTime() - parsedStartDate.getTime()) / (24 * 60 * 60 * 1000)
    );
    if (daysDiff > maxDays) {
      return {
        isValid: false,
